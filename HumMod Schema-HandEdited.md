...
1997 - 2010
Tom Coleman support@biosim.com

Robert Hester rhester@physiology.umsmed.edu

Richard Summers rsummers@emergmed.umsmed.edu
...
physiology.umc.edu/themodelingworkshop/ http://groups.google.com/group/modelingworkshop
...
There are five volumes in this document. The main task of these volumes is to describe all of the XML elements that we have defined for out modeling environment.
...
Some elements do not enclose content. All of their information is represented by a single tag. These are called empty elements. 
...
In addition to text, elements can contain other elements. An example:
   ...
   <curve>
      <name> Name of curve goes here. </name> Additional elements go here.
   </curve>
...
is used to calculate a single numerical value. This value is then used in: 
...
Pi  And we have one literal : PI (3.14159...).
...
BLANK OK for use in logical expressions, but avoid using in mathematical expressions. This number should be displayed as 	(5 underline characters).
...
schema modeltitle structure math control display
...
The model element holds several types of child elements A schema identifier.
A model title.
One or more structure elements (Volume 2). A math element (Volume 2).
A solution control element (Volume 3). Solution display elements (Volume 4).
...
Variables, equations, functions, math blocks and other model components created in a structure have a local name (valid within the structure) and a global name (valid everywhere). You can review naming in Volume 1.
...
Equations are used to develop a relationship around several variables. For instance, in differential equations (diffeq, backwardeuler and
stablediffeq) the declaration names the integral and its derivative. The
integral knows how to determine its own value by tracking values of its derivative.
...
The integrals initial value and integration error limit are floating point.
...
Use the backwardeuler element to declare a differential equation. The underlying numerical method is a partially implicit 1st order Euler algorithm, commonly known as a backward Euler algorithm.
...
The output of a delay slowly follows its input. 


The most common delay is called 1st-order. It is implemented with a differential equation as shown in Equations (1) and (2) and the block diagram below.
...
Use this element to create a lag. Like a delay, the output variable follows the input variable by a 1st-order delay that is determined by the rate constant.
...
We can improve search efficiency by constraining the search. Constraint also allows us to get the correct root when more that one root exists. If the minimum and/or maximum limits are not specified, the search is not constrained in the respective direction.

We can limit the minimum value of the search space in any of three ways.

Name a variable whose value limits the search space. Specify a floating point value that limits the search space. Specify nothing and get no constraint.
...
Two additional steps are needed to fully solve an implicit algebraic equation.
 ...
 1. The relationship between the start variable (YStart) and end variable (YEnd) must be fleshed out in an implicit math block. The start variable is assumed to be defined at the start of the block and the end variable must be defined by the end of the block. This is an implementation of Equation (2) above.

 2. The implicit math block must be called at the correct point in the calculation sequence.
...
This equation is solved using two estimates of Y: the beginning Y (referred to as YStart here) and the ending Y (referred to as YEnd). 
Equation (1) becomes:
...
(2). The starting variable, named in the impliciteq declaration, is defined at the start of the element. The ending variable, also named in the impliciteq declaration, must be defined by this math by the end of the element.
...
Note that our parser does not support recursion, so nested if elements are not allowed in this schema. If you need an if element within an if element, use a andif element for the 2nd if.
...
A lag equation is like a delay equation. The output variable follows the input variable by a 1st-order delay that is determined by the rate constant.
...
We can use a math block as a multiple input ? multiple output funtion. Use the copy element to copy values into the block, call the block, and use the copy element a second time to copy values back out of the block.
...
Note that timer variables can only time out in wrapup code ? where time is known with certainly.
...
The equation solver does four types of math.
 ...
 Parameter Math This math is calculated after a parameter value has been changed. This math defines values that a solely derivatives of parameter values.
...
The order of frequency of calculation types (most to least) is Derivatives
Parameters
Wrapup Context
...
The storage interval is the advance in the independent variable between storage updates. The storage interval determines the granularity of the displayed solution. The storage interval is less than or equal to the display interval. It the current implementation we don?t use a storage interval; instead, we store all solution values.
...
Use this element as a container element for solution control elements. The control element is valid only as a child of the model element.
...
can be used to advance the	solution. A Go menu is shown at right.
...
Use this element to define the initial value of the equation solver?s independent variable, called System.X, usually representing time. The default, and most common, value is 0.
   ...
   <initialx> Initial value of X goes here. </initialx> The value is floating point.
End
...
This is a little tricky, but it works great in tidying up the progress of a solution. A solution interval is specified, but the solution doesn?t go the full interval. Instead, it
goes until the independent variable (System.X) has increased to the next multiple of the specified solution interval.
...
displayint. The default menuitem is an image of the solutionint. End
...
The name of the panel to be displayed first. The font to be used in the panels.
...
<displayfirst> Display this panel 1st. </displayfirst>
...
Default values for name, style, size and color are Tahoma, regular, 9 and
black.
...
A pop menu with name, label and parent menu name (a branch). A menu separator bar.
...
If a bar label is not specified, the variable?s name is used as a label. Default bar and label color is BLACK. I like BLUE.
End
...
Several elements allow the user to change the value of one of the model?s parameters.
The slidebar element creates a slider that can be used to select a new value for the specified parameter.
...
If a label is not specified, the name of the parameter is used as a label. The default height is 1.0 row. The default width is 12 columns.

Sometimes the slidebar width plus the label plus the field width for the value is too wide for the containing group box. This visual draw right through the right side of the group box. You can correct this situation by decreasing the width of the slidebar, using a less verbose label or decreasing the numerical field width (my preference).
...
Current data is written back to the file at application shutdown. The document element is ini.
<ini>
...
Save and load are File menu selections. The usual file extension is ICS.
...
Save and load are File menu selections. The usual file extension is SOLN.
